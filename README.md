...
**Status:** active - image-only baselines complete, publication analysis pack available.
...
**Honest read:** the tile-based image model has the best image-only frozen-feature macro-F1 point estimate and clearer image-level advantages on QWK and the severe-grade endpoint, but several paired deltas still overlap no-difference intervals. Age/sex balanced accuracy is numerically slightly higher than the tile model's cross-validation balanced accuracy. This is a rigorous benchmark with honest uncertainty, not a high-performance clinical model, and it is not framed as one.
...
- The primary image-only models are frozen ImageNet feature extractors plus logistic regression. They are reproducible benchmark floors, not the expected ceiling for fine-tuned dental-radiograph models.
