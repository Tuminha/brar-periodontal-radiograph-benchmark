...
A reproducible, leakage-aware machine-learning benchmark for predicting periodontitis severity (`Level` 1–3) from panoramic radiographs in the public BRAR dataset — with negative-control baselines, calibrated probabilities, and image-level bootstrap comparisons.

**Status:** active — image-only baselines complete, publication analysis pack in progress.
...
Aggregate test metrics across 15 repeated fold evaluations (3 repeats × 5 stratified folds):
...
Secondary endpoint — severe grade (Level 3) vs lower: best image-only model is tile EfficientNet-B0 at balanced accuracy 0.6309 and AUROC 0.6965; adding age/sex reaches balanced accuracy 0.6371.

**Honest read:** the tile-based image model beats the age/sex confounding baseline on macro-F1 with clearer image-level advantages on QWK and the severe-grade endpoint, but several paired deltas still overlap no-difference intervals. This is a rigorous benchmark with honest uncertainty — not a high-performance clinical model, and it is not framed as one.
...
- 988 panoramic radiographs with released image-level severity labels: Level 1 = 170, Level 2 = 560, Level 3 = 258.
...
Numbered scripts (`01`–`15`) run the full pipeline in order; each renders a static HTML/Markdown report under `reports/`. Training ran locally on Apple Silicon (`mps`) with frozen ImageNet encoders — no GPU cluster required.

<details>
<summary><strong>Repository map</strong></summary>

- `scripts/01–15_*.py` — manifest → splits → audits → negative controls → frozen baselines → tile model → publication analysis.
...
1. Improve image-only signal under the same guardrails (current best: tile-based frozen EfficientNet-B0).
2. Finalize the severe-grade secondary endpoint analysis.
